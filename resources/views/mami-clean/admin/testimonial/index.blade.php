...
@section('title', 'Admin - Mami Clean | Testimonial')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title"> Testimonial</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('mami-clean.admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('mami-clean.admin.testimonial.index') }}">Testimonial</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="testimonial_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="testimonial_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3 position-relative form-group">
                            ...
                            <textarea name="testimonial" id="testimonial" rows="5" class="form-control" required></textarea>
                        </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_jabatan" class="control-label">Tetimonial</label>
                        <p id="detail_testimonial"></p>
                    </div>
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_foto" class="control-label">Foto</label>
                        <img src="" alt="" id="detail_foto" class="img-fluid">
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#testimonial_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('mami-clean.admin.testimonial.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#testimonial_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.testimonial.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#testimonial_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#testimonial_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.testimonial.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/mami-clean/testimonial/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $('#detail_testimonial').text(data.result.testimonial);
                    $('#detail_foto').attr('src', "{{ asset('images/mami-clean/testimoni') }}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/mami-clean/testimonial/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
                    $('#nama').val(data.result.nama);
                    $('#jabatan').val(data.result.jabatan);
                    $('#testimonial').val(data.result.testimonial);
                    var lokasi_foto = "{{ asset('images/mami-clean/foto-testimoni') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/mami-clean/testimonial/destroy') }}"+'/'+user_id,
                beforeSend: function(){
