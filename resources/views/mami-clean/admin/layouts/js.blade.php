...
<script>
    const settings = new Settings({
        @if (Auth::guard('mami_clean')->check())
            attributes: {
                color: "{{Auth::guard('mami_clean')->user()->color_layout?Auth::guard('mami_clean')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('mami_clean')->user()->nav_color?Auth::guard('mami_clean')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('mami_clean')->user()->behaviour?Auth::guard('mami_clean')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('mami_clean')->user()->layout?Auth::guard('mami_clean')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('mami_clean')->user()->radius?Auth::guard('mami_clean')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('mami_clean')->user()->placement?Auth::guard('mami_clean')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('mami-clean.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
