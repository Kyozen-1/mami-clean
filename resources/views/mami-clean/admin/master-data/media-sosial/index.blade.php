...
@section('title', 'Admin - Mami Clean | Master Media Sosial')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('mami-clean.admin.master-data.media-sosial.index') }}">Master Media Sosial</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_media_sosial_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('mami-clean.admin.master-data.media-sosial.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_media_sosial_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.master-data.media-sosial.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.master-data.media-sosial.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/mami-clean/master-data/media-sosial/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/mami-clean/master-data/media-sosial/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/mami-clean/master-data/media-sosial/destroy') }}"+'/'+user_id,
                beforeSend: function(){
