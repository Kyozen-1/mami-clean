...
@section('title', 'Admin - Mami Clean | Brand')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('mami-clean.admin.brand.index') }}">Brand</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#brand_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('mami-clean.admin.brand.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#brand_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.brand.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('mami-clean.admin.brand.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/mami-clean/brand/detail/') }}" + '/' + id,
                dataType: "json",
                success: function(data)
                {
                    $('#detail-title').text('Detail Data');
                    $('#detail_nama').val(data.result.nama);
                    $('#detail_foto').attr('src', "{{ asset('images/mami-clean/brand') }}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/mami-clean/brand/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
                    $('#nama').val(data.result.nama);
                    var lokasi_foto = "{{ asset('images/mami-clean/brand') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            $.ajax({
                url: "{{ url('/mami-clean/brand/destroy') }}"+'/'+user_id,
                beforeSend: function(){
